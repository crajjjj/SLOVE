...
| `intense_from_bar_only` | `0` | How the "intense" state is decided. `0` (default): intense when **either** the enjoyment bar crosses `intenseenjoyment` **or** the stage is authored fast/intense (HentaiRim `1F` label). `1`: **SLSO-style bar-only** — drops the authored fast-tag path entirely, so the enjoyment bar **alone** decides intensity and `NearOrgasmNoises` plays only at/above `intenseenjoyment`, and for no other reason. Use this to reclaim SLSO's granular meter control when most animations read as intense from their motion tags. **Both P+ and classic.** Only affects **voice** — facial expressions stay tag-driven. |
