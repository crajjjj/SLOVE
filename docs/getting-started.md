...
| **[AudioUtil](https://crajjjj.github.io/AudioUtil/) 0.9.3+** | the audio engine: folder-based voice playback, lipsync, and the TOML reader every SLO VE setting is read through. **Without it there is no voice engine.** **0.9.3 or newer is required** — SLO VE calls its `GetSlotVariation` native (API v2) to drive per-pack Variation-A/B dispatch; on older builds that call fails to bind and voice routing breaks. **0.9.4+ recommended**: `AuditVoicePack` then attributes each category to the slot that actually supplies it (in-pack vs stock backfill). |
